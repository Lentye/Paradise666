GLOBAL_LIST_EMPTY(achievement_data)

/datum/achievement_data
	var/name = "Achievements Panel"
	...
	var/static/list/icon3 = list(
		"default", "bbgum", "colossus", "drake",
		"firstboss", "hierophant", "legion", "miner", "swarmer", "smoke",
		"tendril"
		)

/datum/achievement_data/New(ckey)
	owner_ckey = ckey
	if(SSmedals.initialized && !initialized)
		InitializeData()
...
/datum/achievement_data/proc/get_changed_data()
	. = list()
	for(var/T in data)
		var/datum/award/A = SSmedals.awards[T]
...
/datum/achievement_data/proc/get_data(achievement_type)
	var/datum/award/A = SSmedals.awards[achievement_type]
	if(!A.name)
		return FALSE
...
/client/verb/checkachievements()
	...
	if(!SSmedals.initialized)
		to_chat(usr, "<span class='boldannounce'>Subsystem not initialized yet,FUCK OFF</span>")
		return
	achievements.ui_interact(usr)

/datum/achievement_data/ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, force_open = FALSE, datum/tgui/master_ui = null, datum/ui_state/state = GLOB.admin_state)
	ui = SStgui.try_update_ui(user, src, ui_key, ui, force_open)
	if(!ui)
		ui = new(user, src, ui_key, "Achievements", "Achievements Panel", 540, 680, master_ui, state)
		ui.autoupdate = TRUE
		ui.open()

/datum/achievement_data/ui_data(mob/user)
	var/ret_data = list()
	ret_data["categories"] = list("Bosses", "Scores")
...
/datum/achievement_data/ui_static_data(mob/user)
	...
	for(var/score in SSmedals.scores)
		...
		if(!S.name || !S.track_high_scores || !S.high_scores.len)
			continue
		data["highscore"] += list(list("name" = S.name,"scores" = S.high_scores))

	data["allStylesPreview"] = list()
	for (var/style in icon3)
		var/icon/true_icon = icon('icons/achievements.dmi', style, SOUTH, frame = 1)
		data["allStylesPreview"][style] = icon2base64(true_icon)
